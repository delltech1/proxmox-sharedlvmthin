...
The guardian consumes a canonical, read-only inventory generated by
`sharedlvmthin-thin-guard-inventory`.  For each managed Thin pool it must
positively correlate the pinned storage/VG UUID, owner node and owner epoch
tags, exact device-mapper name and UUID, and an attached volume reference from
a locally running QEMU process.  Duplicate, missing, malformed or conflicting
evidence is an error; an empty-looking result is never substituted for a
failed probe.

Two-node clusters without a QDevice remain supported for planned operation
while both nodes are online and quorate.  The inventory does not manufacture
failover authority: a one-node survivor needs restored quorum and positive
