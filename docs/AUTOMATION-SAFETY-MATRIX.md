...
| Thick snapshot/rollback prepare | Persist exact VG intent, then atomically create each LV with complete signed ownership and `autoactivation=n`; explicitly remove an unpublished signed remainder with `thick-recover-prepare` | Recovery requires the unchanged MATERIALIZED anchor and frontend, no transition runtime, the exact OPEN transaction, and only its correctly signed destination/metadata objects; ambiguity preserves everything | PREPARED or later transitions use state-specific resume; no object is adopted by name alone |
